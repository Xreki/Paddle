  ...
  if (is_sender) {
    ...
    for (int64_t token_idx = token_start_idx; token_idx < token_end_idx;) {
      ...
      while (chunk_token_idx < num_max_send_tokens and
        ...
        // be different from the warp assigned to send subsequent data
        if (send_lane_id == 0 and
        ...
        if (cached_channel_tail_idx % num_send_warps_per_rank ==
          ...
          if (send_lane_id < num_topk) {
            ...
            channel_topk_weights_buffers[dst_slot_idx * num_topk +
                                         send_lane_id] = weight_value;
          }
...
#pragma unroll
    ...
    while (recv_lane_id == 0 and (total_offset = ld_volatile_global(
                                      channel_start_offset.buffer())) == 0)
      ;
    while (recv_lane_id == 0 and (num_tokens_to_recv = ld_volatile_global(
    ...
    while (num_tokens_to_recv > 0) {
      ...
      // indices hold by different warps are same
...
#pragma unroll 4
      ...
      asm volatile("bar.sync %0, %1;" ::"r"(responsible_rank),
                   "r"(num_threads_per_rank));
      if (recv_warp_id_in_rank == num_recv_warps_per_rank - 1 and
          recv_lane_id == 0)
...
#pragma unroll
      ...
      for (int64_t token_idx = token_start_idx + recv_warp_id - 1;
        ...
        auto start_time = clock64();
        while (channel_tail_idx[recv_lane_id] <= expected_head and
               expected_head >= 0) {
          // Timeout check
          if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
            printf(
